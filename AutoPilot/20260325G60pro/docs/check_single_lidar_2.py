#!/usr/bin/env python3
"""
检查左后单线激光雷达（LakiBeam1S）的点云角度和距离信息
话题: /lidar/single_2/scan
传感器: 192.168.2.150 (左后)
"""
import rclpy
...
class SingleLidar2Checker(Node):
    def __init__(self):
        super().__init__('single_lidar_2_checker')
        ...
        self.subscription = self.create_subscription(
            ...
            qos_profile)
        self.count = 0
        self.get_logger().info('等待左后单线激光雷达数据 (/lidar/single_2/scan)...')
    ...
    def scan_callback(self, msg):
        ...
        print(f"\n{'='*60}")
        print(f"{'左后单线激光雷达 (LakiBeam1S) 数据分析':^60}")
        print(f"{'='*60}")
        print(f"话题: /lidar/single_2/scan")
        print(f"传感器 IP: 192.168.2.150")
        ...
        if valid_points:
            ...
            print("可能原因:")
            print("  1. 雷达未启动或未连接")
            print("  2. 网络配置错误 (检查 192.168.2.150)")
            print("  3. 雷达视野内无障碍物")
